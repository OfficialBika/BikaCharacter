...
from config import CARD_DATABASE_CHANNEL_ID, RARITY_ORDER
...
async def _ensure_card_counter() -> None:
    ...
    try:
        await db.counters.insert_one({"_id": CARD_COUNTER_ID, "seq": max_id, "updatedAt": utcnow()})
    except Exception:
...
def _database_caption(action: str, parsed: dict, adder) -> str:
    # action must be exactly Saved or Update for clear channel history.
...
async def _post_to_card_database_channel(context: ContextTypes.DEFAULT_TYPE, file_id: str, caption: str) -> dict:
...
async def photo_add_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    db = get_db()
    existing = await db.photos.find_one({"cardId": parsed["cardId"]}, {"_id": 1, "createdAt": 1})
...
def register_photo_add_handlers(app: Application) -> None:
    app.add_handler(MessageHandler(filters.ChatType.PRIVATE & filters.PHOTO, photo_add_handler))
